...
const Dashboard = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        setData(res.data);
        setNMPIPG(
          Object.entries(res.data.nombre_de_mutations_par_impact_par_gène).map(
        ...
        setDPTM(
          Object.entries(res.data.distribution_par_type_de_mutation).map(
            ([name, value]) => ({ name, "nombre de mutation": value })
          )
        );
        setNMPIPC(
          Object.entries(
  ...
  //       // console.log("key: ", key, "listObj[i].name: ", listObj[i]);
  //       if (key === listObj[i].name) {
  //         list.push(listObj[i]);
  //       }
  //     }
  //   }
